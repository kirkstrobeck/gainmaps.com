...
function getReports(preset: "desktop" | "mobile") {
  ...
  // sort by mtime descending, take the 18 most recent
  const withMtime = files.map(f => ({
    f,
    mtime: statSync(resolve(dir, f)).mtimeMs
  }));
  withMtime.sort((a, b) => b.mtime - a.mtime);
  const recent = withMtime.slice(0, 18).map(x => x.f).sort();

  console.log(`\n## ${preset.toUpperCase()} REPORTS (18 most recent of ${files.length} files)\n`);
