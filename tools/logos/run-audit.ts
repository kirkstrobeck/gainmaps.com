...
async function auditShipped(seed: LogoSeed, path: string): Promise<Row> {
  const metric = await shippedLogoMetric(await readFile(path)).catch((error: unknown) => errorMessage(error));
  ...
  return {
    seed, input: "shipped", source: "shipped", fileName: "logo.svg", metric, decision,
...
async function main(): Promise<void> {
  ...
  const shipped = LOGO_SEEDS.filter((seed) => existsSync(join(PUBLIC, seed.slug, "logo.svg")));
  ...
  const shippedRows = await mapChunked(shipped, FETCH_CONCURRENCY, (seed) => auditShipped(seed, join(PUBLIC, seed.slug, "logo.svg")));
  ...
  const rows = [...shippedRows, ...fetchedRows]
    .sort((left, right) => LOGO_SEEDS.indexOf(left.seed) - LOGO_SEEDS.indexOf(right.seed));
  const header = [
    "# schema=logo-audit-v2",
    "# shipped rows use checked-in logo.svg; fetched rows use strip+normalize(source); this tool never writes apps/web/public/logos",
    ...
    `# threshold=${SCORE_MIN}; derivation remains disabled while the required gate is infeasible`,
  ...
  await writeFile(join(LOG_DIR, "ink-audit.json"), JSON.stringify({ schema: "logo-audit-v2", rows }, null, 2) + "\n");
  console.log(log);
}
