...
describe("output-path", () => {
  ...
  it("outputs .jpg for non-JPEG inputs (.PNG)", () => {
    assert.match(defaultOutputPath("/tmp/photo.PNG"), /photo-gainmap\.jpg$/);
    const plan = planOutputs(["/tmp/a.PNG"], { suffix: "-gainmap", stdout: false, outputIsDirectory: false });
    assert.match(plan[0]!.output ?? "", /a-gainmap\.jpg$/);
  });
});
